...
function SmartHome() {
  ...
  useEffect(()=>{
    ...
    axios.get("https://rent-mojo-server.onrender.com/smarthome").then((res)=>{
      setData(res.data)
      setClose(false)
    })
  },[])
          ...
          <Box sx={{display:"grid",gridTemplateColumns:{xl:"repeat(2,1fr)",lg:"repeat(1,1fr)"},gap:"15px",height:"130vh",overflow:"scroll","&::-webkit-scrollbar":{width:"1px"}}}>
            {
              data && data.map((el)=>{
                return(
                  <Link to={`/packages/smarthome/${el.id}`} style={{textDecoration:"none"}}>
