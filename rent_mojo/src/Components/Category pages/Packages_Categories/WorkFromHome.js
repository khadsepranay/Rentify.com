...
function WorkFromHome() {
  ...
  useEffect(()=>{
    ...
    axios.get("http://localhost:8000/wfh").then((res)=>{
      setData(res.data)
      setClose(false)
    })
  },[])
          ...
          <Box sx={{display:"grid",gridTemplateColumns:{xl:"repeat(2,1fr)",lg:"repeat(1,1fr)"},gap:"35px",height:"100vh",overflow:"scroll","&::-webkit-scrollbar":{width:"1px"}}}>
            {
              data && data.map((el)=>{
                return(
                  <Link to={`/wfh/${el.id}`} style={{textDecoration:"none"}}>
